...
class MainWindow(QMainWindow, Ui_MainWindow):
	...
	sendTo = sendTo[1:]	
	pw = pwMappingParams.PwytterMapping(self._store)
	pw.readFromXML()	
	keys = pw.values.keys()
	if sendTo in keys:
	
		sendTo = pw.values[sendTo]
	self.username = pw.values['username']
	self.password = pw.values['password']
	id = xmpp.protocol.JID(self.username)
        self.jabber = xmpp.Client(id.getDomain(),debug=[])
        con = self.jabber.connect(("talk.google.com",5222))
        if not con:
	...
	sendTo = sendTo + "@xmpp.us"
		
	msg = xmpp.protocol.Message(to= sendTo , body=text , typ='chat')
