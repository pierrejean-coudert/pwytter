...
class MainWindow(QMainWindow,Ui_BuddyList):
	...
	self.pw = pwMappingParams.PwytterMapping(self.store)
        try :
           self.pw.readFromXML()
        except Exception ,e :
           pass

	self.pw['username'] = self.parent.username
        self.pw['password'] = self.parent.password
...
class ChatWindow(QMainWindow,Ui_ChatWindow):
	...
	def send(self):
		...
		"""
		sendtolist = []
		pos = self.sendTo.find('@')
		sendto = self.sendTo[:pos]
			...
			try :
				...
				keyvals = self.pw.values.keys()
				
				for frndskey in keyvals :
					id = self.pw.values[frndskey]
...
class TwitterAccount(QDialog,Ui_Map) :
	...
	def __init__(self,parent) :
		...
		self.username = parent.parent.username
		self.passwd = parent.parent.password
